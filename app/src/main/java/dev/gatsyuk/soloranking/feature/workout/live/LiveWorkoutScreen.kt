package dev.gatsyuk.soloranking.feature.workout.live
...
import dev.gatsyuk.soloranking.core.model.SetField
...
import dev.gatsyuk.soloranking.core.model.trackedDurationMillis
import dev.gatsyuk.soloranking.core.ui.ActionBottomSheet
import dev.gatsyuk.soloranking.core.ui.SheetAction
import dev.gatsyuk.soloranking.core.ui.exerciseTypeLabel
import dev.gatsyuk.soloranking.feature.workout.components.ExercisePickerSheet
...
fun LiveWorkoutScreen(
    ...
    val catalogViewModel: dev.gatsyuk.soloranking.feature.workout.components.ExerciseCatalogViewModel =
        hiltViewModel()
    ...
    newExercisePrefill?.let { prefill ->
        ...
        dev.gatsyuk.soloranking.feature.workout.components.ExerciseFormDialog(
            muscleGroups = groups,
            initialName = prefill,
...
private fun WorkoutHeader(
    workout: dev.gatsyuk.soloranking.core.database.entity.WorkoutEntity,
    unit: WeightUnit,
    onUpdate: (dev.gatsyuk.soloranking.core.database.entity.WorkoutEntity) -> Unit,
    onFinish: () -> Unit,
) {
...
private fun ExerciseSection(
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(12.dp)) {
            ...
            // Ghost values: last session's numbers show greyed in empty fields,
            // aligned row-for-row with the previous performance.
            val ghosts = remember(lastSessionSets) { lastSessionSets.sortedBy { it.position } }
            var workingIndex = 0
...
private fun MeasurementField(
    ...
    // Last session's number, greyed. Visible while the field is empty; typing
    // hides it, erasing brings it back (reference-app behavior).
    val ghostText = ghost?.let { initialText(it, field, unit) }.orEmpty()

    OutlinedTextField(
        value = text,
        placeholder = if (ghostText.isEmpty()) null else {
            {
                Text(
                    ghostText,
